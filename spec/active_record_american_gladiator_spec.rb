...
describe "ActiveRecord American Gladiator" do
  context "Joust" do
    it "loads all items" do
      item_1 = Item.create(name: "Pugil Sticks")
      item_2 = Item.create(name: "Trap Door")
      item_3 = Item.create(name: "Crash Pad", status: "inactive")
  ...
  context "Powerball" do
    it "returns all items containing Powerball" do
      ...
      # Changeable Start
      items = Item.where("name LIKE :name", name: "%Powerball%")
      # Changeable End
  ...
  context "Breakthrough and Conquer" do
    it "returns all orders with Footballs and Wrestling Rings" do
      ...
      orders = Order.joins(:items)
                    .where('items.name = ? OR items.name = ?', 'Wrestling Ring', 'Football')
      # Changeable End
  ...
  context "The Eliminator" do
    it "returns all orders placed 2 weeks ago" do
      ...
      # Changeable Start
      orders = Order.where('created_at < ?', last_week).where('created_at > ?', two_weeks_ago)
      # Changeable End
